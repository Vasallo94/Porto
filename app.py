...
def main():
    ...
    with col2:
        st.title("Airbnb: Oporto")
    with col3:
        image = Image.open("img/porto.jpeg")

        st.image(image, caption="Porto by prettymaps", use_column_width="auto")
    ...
    with tab_plots:
        ...
        with cols[1]:
            ...
            mean_prices = (
                df_slider.loc[df_slider["accommodates"] == 2]
            ...
            def style_function(feature):
                return {
                    "fillColor": color_scale(
                        price_dict.get(feature["properties"]["neighbourhood"], 0)
                    ),
                    "color": "black",
                    "weight": 1,
                    "dashArray": "5, 5",
        ...
        with cols[0]:
            ...
            HeatMap(
                data=df_slider[["latitude", "longitude", "price"]],
                radius=20,
        ...
        with cols[1]:
            ...
            st.write("Mapa de precios en 3D")
            st.pydeck_chart(
                pdk.Deck(
                    map_style=None,
    ...
    with tab_plots:
        ...
        cols = st.columns(2)
        with cols[0]:
            feq = df_slider["neighbourhood"].value_counts().sort_values(ascending=True)
            feq = feq[feq > 500]

            fig1 = px.bar(
                feq, x=feq.index, y=feq.values, orientation="h", template="plotly_dark"
            )  # Fix: Swap x and y parameters
            fig1.update_layout(
        ...
        with cols[1]:
            prop = df_slider.groupby(["property_type", "room_type"]).room_type.count()
            prop = prop.unstack()
            ...
            proper.update_layout(
                title="Tipos de alojamientos en Oporto",
                xaxis_title="Número",
                yaxis_title="",
                legend_title="",
                font=dict(size=14),
        ...
        with cols[0]:
            ...
            room_by_type.update_layout(
                title="Número de reservas por tipo de alojamiento",
                xaxis_title="Número",
                yaxis_title="",
            )
        ...
        with cols[1]:
            feq = df_slider["accommodates"].value_counts().sort_index().reset_index()
            ...
            accomm.update_layout(
                title={"text": "Número de huéspedes", "x": 0.5},
                xaxis_title="Huéspedes",
                yaxis_title="Número",
                font=dict(size=14),
            )
    ...
    with tab_plots:
        ...
        with cols[0]:
            ...
            feq = (
                feq.groupby("neighbourhood")["price"]
                .mean()
        ...
        with cols[1]:
            feq1 = (
                df_slider[df_slider["number_of_reviews"] >= 10]
                .groupby("neighbourhood")["review_scores_location"]
                .mean()
                .sort_values(ascending=True)
            )
            ...
            fig1.update_layout(
                xaxis_title="Nota (1-5)",
                yaxis_title="",
                title="Nota por localización",
            )
        ...
        df_frequencies = (
            df_slider["host_is_superhost"].value_counts(normalize=True).reset_index()
        )
        ...
        fig_super.update_traces(texttemplate="%{y:.2f}%", textposition="inside")
        fig_super.update_layout(uniformtext_minsize=8, uniformtext_mode="hide")
        fig_super.update_layout(
            legend_title="Superhost",
            legend=dict(
        ...
        sh_v_h = px.scatter(
            ...
            labels={"host_is_superhost": "Superhost", "price": "Precio"},
            template="plotly_dark",
        )
        sh_v_h.update_layout(
            title="Precios de Superhost", xaxis=dict(tickmode="linear")
        )
    ...
    with tab_plots:
        ...
        numeric_columns = df_cal.select_dtypes(include=[np.number]).columns
        sum_available = (
        ...
        # Gráfico del precio medio para dos personas
        numeric_columns = df_cal.select_dtypes(include=[np.number]).columns
        average_price = (
            df_cal[(df_cal["available"] == "t") & (df_cal["accommodates"] == 2)]
            .groupby(["date"])[numeric_columns]
            .mean()
            .astype(np.int64)
            .reset_index()
        ...
        precio = px.line(
            ...
            y="price_x",
            title="Precio medio para dos personas",
        )
        precio.update_traces(text=average_price["weekday"])
        precio.update_layout(
            xaxis_title="Fecha", yaxis_title="Precio", template="plotly_dark"
        )
    ...
    with tab_plots:
        ...
        for category in categories:
            ...
            if scores_col.dtype != "float64":
                scores_col = pd.to_numeric(scores_col, errors="coerce").fillna(0)
            scores = go.Figure(go.Histogram(x=scores_col, nbinsx=800, histfunc="count"))
